...
async function fetchEntries(): Promise<LexisEntry[]> {
  if (isLocalStorageMode()) return loadLocalEntries();
...
export function useLexicon() {
  ...
  const addMutation = useMutation({
    mutationFn: async (entry: LexisEntryInput) => {
      if (isLocalStorageMode()) {
        const createdEntry = normalizeEntry({ ...entry, id: crypto.randomUUID(), createdAt: Date.now() });
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, updates }: { id: string; updates: Partial<LexisEntryInput> & { grammar?: LexisEntry["grammar"] | null } }) => {
      if (isLocalStorageMode()) {
        const nextEntries = loadLocalEntries().map((entry) => {
          if (entry.id !== id) return entry;
          const merged: LexisEntry = { ...entry, ...updates };
          if (updates.grammar === null) delete merged.grammar;
          else if (updates.grammar !== undefined) merged.grammar = updates.grammar;
      ...
      if ("grammar" in updates) body.grammar = updates.grammar === undefined || updates.grammar === null ? null : updates.grammar;
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ENTRIES_QUERY_KEY }),
    onError: (error) => toast.error(error instanceof Error ? error.message : "Could not update entry."),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ENTRIES_QUERY_KEY }),
    onError: (error) => toast.error(error instanceof Error ? error.message : "Could not delete entry."),
  });
  ...
  const findMatches = useCallback((query: string): LexisEntry[] => {
    ...
    return allEntries.filter((entry) => {
      const grammarHaystack = entry.grammar
        ? Object.values(entry.grammar).filter(Boolean).join(" ").toLowerCase()
        : "";
      return (
  ...
  const findLinkedWords = useCallback((entry: LexisEntry): LexisEntry[] => {
    ...
    const words = [entry.danish, entry.english]
    ...
    return allEntries.filter(
      (candidate) =>
        ...
        [candidate.danish, candidate.english].some((field) => words.includes(field.toLowerCase())),
    );
  }, [allEntries]);
